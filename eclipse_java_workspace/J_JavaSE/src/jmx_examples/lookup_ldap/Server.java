...
//        //InitialContext root = new InitialLdapContext(env,null);
        ...
        //------我的新增加
        Hashtable env_ladp = new Hashtable();//或者使用Properties,或者把这些写入jni.properties文件中,再去读
        env_ladp.put(Context.INITIAL_CONTEXT_FACTORY,"com.sun.jndi.ldap.LdapCtxFactory");
        env_ladp.put(Context.PROVIDER_URL,			"ldap://127.0.0.1:10389/dc=example,dc=com");//??????
        env_ladp.put(Context.SECURITY_PRINCIPAL,	"uid=admin,ou=system");
        env_ladp.put(Context.SECURITY_CREDENTIALS,	"secret");
        
    	/*
        Bind DN or user:uid=admin,ou=system
    	Bind password:secret
    			
		修改密码使用 Aapache Directory Studio
		在ou=system/uid=admin级下 ,显示objectClass=person(structual),userPassword中修改密码
*/
        
//        InitialContext root = new InitialLdapContext(env_ladp,null);
//        return (DirContext)(root.lookup(""));
        return new InitialLdapContext(env_ladp,null);
    }
    ...
    public static void register(DirContext root,
	    ...
	    // a javaContainer with the jmxConnector auxiliary class.
            ...
            objclass.add("jmxConnector");//要导入.ldif文件才会有,
            attrs.put(objclass);
...
//	}
    ...
    public JMXConnectorServer rmi(String url) 
        ...
        //
        final HashMap env = new HashMap(); //或者使用Properties,或者把这些写入jni.properties文件中,再去读
//        final String rprop = RMIConnectorServer.JNDI_REBIND_ATTRIBUTE; //JMX专用的,值可是"true"
...
//          final String url = System.getProperty("url", "service:jmx:rmi:///jndi/ldap://127.0.01:10389/dc=example,dc=com");//OK
            final String url = System.getProperty("url", "service:jmx:iiop:///jndi/ldap://127.0.01:10389/dc=example,dc=com");//OK
            JMXServiceURL jurl=new JMXServiceURL(url);
