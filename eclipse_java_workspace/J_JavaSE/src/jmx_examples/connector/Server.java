...
public class Server {
    ...
    public static void main(String[] args) {
	    ...
	    String domain = mbs.getDefaultDomain();//值是DefaultDomain
	    echo("\tDefault Domain = " + domain);
	    waitForEnterPressed();

	    // Create and register the SimpleStandard MBean
	    //
	    String mbeanClassName = "jmx_examples.connector.SimpleStandard";//修改为带包名,正确的类名路径
	    String mbeanObjectNameStr =domain + ":type=" + mbeanClassName + ",name=1";//name 为1
	    ...
	    //
	    mbeanClassName = "jmx_examples.connector.SimpleDynamic";//修改为带包名,正确的类名路径
	    mbeanObjectNameStr =
	    ...
	    waitForEnterPressed();
            ...
            //JMXServiceURL url = new JMXServiceURL( "service:jmx:rmi:///jndi/rmi://localhost:9999/server");//OK
           
            //JMXConnectorServer start()之前要 orbd -ORBInitialPort 7777 & JDK自带命令,会在当前目录生成文件orb.db目录,第二次测试时要删除目录
            JMXServiceURL url = new JMXServiceURL( "service:jmx:iiop:///jndi/iiop://localhost:7777/server");
	...
	try {
	    ObjectName mbeanObjectName = ObjectName.getInstance(mbeanObjectNameStr);//不是new,name 的值只是个标识
	    mbs.createMBean(mbeanClassName, mbeanObjectName);//mbeanClassName要是可以找到的类,不是registerMBean了
	...
	try {
	    ...
	    String State = (String) mbs.getAttribute(mbeanObjectName, "State");//开头第一个是大写的,对应getState()
	    Integer NbChanges =
